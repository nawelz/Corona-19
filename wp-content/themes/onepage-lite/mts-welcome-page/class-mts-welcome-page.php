...
if ( ! class_exists( 'MTS_welcome_page' ) ) {
	/**
	 * Singleton class used for generating the Welcome page of the theme.
	 */
	class MTS_welcome_page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The MTS_welcome_page class version.
		 */
		 ...
		 *
		 * @var string $page_name The title of the welcome page.
		 */
		private $page_name;
		/**
		...
		/**
		 * The single instance of MTS_welcome_page
		 *
		 * @var MTS_welcome_page $instance The  MTS_welcome_page instance.
		 */
		private static $instance;

		/**
		 * The Main MTS_welcome_page instance.
		 *
		 * We make sure that only one instance of MTS_welcome_page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof MTS_welcome_page ) ) {
				self::$instance = new MTS_welcome_page;
		...
		public function setup_actions() {
			...
			/* enqueue script and style for welcome page */
			add_action( 'admin_enqueue_scripts', array( $this, 'style_and_scripts' ) );

			/* ajax callback for dismissable required actions */
			add_action( 'wp_ajax_mts_welcome_page_dismiss_required_action', array( $this, 'dismiss_required_action_callback' ) );
			add_action( 'wp_ajax_nopriv_mts_welcome_page_dismiss_required_action', array( $this, 'dismiss_required_action_callback') );
		}
		...
		public function activation_admin_notice() {
			...
			if ( is_admin() && ( 'themes.php' == $pagenow ) && isset( $_GET['activated'] ) ) {
				add_action( 'admin_notices', array( $this, 'mts_welcome_page_welcome_admin_notice' ), 99 );
			}
		}

		/**
		 * Display an admin notice linking to the welcome page
		 */
		public function mts_welcome_page_welcome_admin_notice() {
		 ...
		 */
		public function mts_welcome_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap mts-wrap">';
				...
				echo '<a href="https://mythemeshop.com/" target="_blank" class="wp-badge mts-welcome-logo"></a>';
		...
		public function call_plugin_api( $slug ) {
			...
			if ( false === ( $call_api = get_transient( 'mts_welcome_page_plugin_information_transient_' . $slug ) ) ) {
				...
				set_transient( 'mts_welcome_page_plugin_information_transient_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
			}
		...
		public function get_plugin_icon( $arr ) {
			...
			if ( ! empty( $arr['svg'] ) ) {
				...
				$plugin_icon_url = get_template_directory_uri() . '/mts-welcome-page/images/placeholder_plugin.png';
			}
		...
		public function recommended_actions() {
			...
			if ( ! empty( $recommended_actions ) ) {
				...
				if ( ! empty( $actions ) && is_array( $actions ) ) {
					...
					$mts_welcome_page_show_required_actions = get_option( $this->theme_slug . '_required_actions' );
					...
					foreach ( $actions as $action_key => $action_value ) {
						...
						if ( @$mts_welcome_page_show_required_actions[ $action_value['id'] ] === false ) {
							$hidden = true;
						}
						...
						echo '<div class="mts-welcome-page-action-required-box">';

						if ( ! $hidden ) {
							echo '<span data-action="dismiss" class="dashicons dashicons-visibility mts-welcome-page-required-action-button" id="' . esc_attr( $action_value['id'] ) . '"></span>';
						} else {
							echo '<span data-action="add" class="dashicons dashicons-hidden mts-welcome-page-required-action-button" id="' . esc_attr( $action_value['id'] ) .'"></span>';
						}
			...
			if ( ! empty( $recommended_plugins ) ) {
				if ( ! empty( $recommended_plugins['content'] ) && is_array( $recommended_plugins['content'] ) ) {
					...
					foreach ( $recommended_plugins['content'] as $recommended_plugins_item ) {
						...
						//print_r($recommended_plugins_item);
						...
						if ( ! empty( $recommended_plugins_item['slug'] ) ) {
							...
							if ( ! empty( $info->sections ) ) {
								echo '<p class="description">'. mts_truncate( wp_strip_all_tags( $info->sections['description'] ), 100 ).'...</p>';
							}
		...
		public function child_themes() {
			echo '<div id="child-themes" class="mts-welcome-page-tab-pane">';
			...
			if ( ! empty( $child_themes ) ) {
				if ( ! empty( $child_themes['content'] ) && is_array( $child_themes['content'] ) ) {
					echo '<div class="mts-welcome-row">';
					for ( $i = 0; $i < count( $child_themes['content'] ); $i ++ ) {
						...
						if ( ! empty( $child['image'] ) ) {
							echo '<div class="mts-welcome-child-theme">';
							echo '<div class="mts-welcome-page-child-theme-image">';
							echo '<img src="' . esc_url( $child['image'] ) . '" alt="' . ( ! empty( $child['image_alt'] ) ? esc_html( $child['image_alt'] ) : '' ) . '" />';
							if ( ! empty( $child['title'] ) ) {
								echo '<div class="mts-welcome-page-child-theme-details">';
							...
							echo '</div><!--mts-welcome-page-child-theme-image-->';
							echo '</div><!--mts-welcome-child-theme-->';
						}// End if().
					}// End for().
					echo '</div>';
				}// End if().
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<div id="free_pro" class="mts-welcome-page-tab-pane mts-welcome-page-fre-pro">';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						echo '<tr class="mts-welcome-page-text-center">';
						echo '<td></td>';
		...
		/**
		 * Load css and scripts for the welcome page
		 */
		public function style_and_scripts( $hook_suffix ) {

			// this is needed on all admin pages, not just the welcome page, for the badge action count in the wordpress main sidebar
			wp_enqueue_style( 'mts-welcome-page-css', get_template_directory_uri() . '/mts-welcome-page/css/mts_welcome_page.css' );

			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {

				wp_enqueue_script( 'mts-welcome-page-js', get_template_directory_uri() . '/mts-welcome-page/js/mts_welcome_page.js', array( 'jquery' ) );
				...
				wp_localize_script( 'mts-welcome-page-js', 'tiWelcomePageObject', array(
					'nr_actions_required'      => count( $required_actions ),
					'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
					'template_directory'       => get_template_directory_uri(),
					'activating_string'        => __( 'Activating', 'onepage-lite' )
				) );
			...
			if ( ! empty( $action_id ) ) {
				...
				if ( get_option( $this->theme_slug . '_required_actions' ) ) {
					...
					switch ( esc_html( $_GET['todo'] ) ) {
						case 'add';
							$mts_welcome_page_show_required_actions[ absint( $action_id ) ] = true;
							break;
						case 'dismiss';
							$mts_welcome_page_show_required_actions[ absint( $action_id ) ] = false;
							break;
					}

					update_option( $this->theme_slug . '_required_actions', $mts_welcome_page_show_required_actions );

					/* create the new option,with false for the specified id */
				} else {

					$mts_welcome_page_show_required_actions_new = array();

					if ( ! empty( $recommended_actions ) ) {

						foreach ( $recommended_actions as $mts_welcome_page_required_action ) {

							if ( $mts_welcome_page_required_action['id'] == $action_id ) {
								$mts_welcome_page_show_required_actions_new[ $mts_welcome_page_required_action['id'] ] = false;
							} else {
								$mts_welcome_page_show_required_actions_new[ $mts_welcome_page_required_action['id'] ] = true;
							}

						}

						update_option( $this->theme_slug . '_required_actions', $mts_welcome_page_show_required_actions_new );
