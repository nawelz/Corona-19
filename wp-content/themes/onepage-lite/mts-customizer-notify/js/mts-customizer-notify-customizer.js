...
(function (api) {
    ...
    api.sectionConstructor['mts-customizer-notify-section'] = api.Section.extend({
...
jQuery(document).ready(function () {
    ...
    jQuery('.mts-customizer-notify-dismiss-recommended-action').click(function () {

        var id = jQuery(this).attr('id'),
            action = jQuery(this).attr('data-action');
        jQuery.ajax({
            type: 'GET',
            data: {action: 'mts_customizer_notify_dismiss_recommended_action', id: id, todo: action},
            dataType: 'html',
            ...
            success: function (data) {
                ...
                var recommended_sction = jQuery('#accordion-section-mts_customizer_notify_recomended_actions');
                var actions_count = recommended_sction.find('.mts-customizer-notify-actions-count');
                var section_title = recommended_sction.find('.section-title');
                jQuery('.mts-customizer-notify-actions-count .current-index').text(index);
                ...
                if (jQuery('.recomended-actions_container > .epsilon-recommended-actions').length === 0) {
                    ...
                    if (jQuery('.recomended-actions_container > .epsilon-recommended-plugins').length === 0) {
                        jQuery('.control-section-mts-customizer-notify-recomended-actions').remove();
                    } else {
    ...
    jQuery('.mts-customizer-notify-dismiss-button-recommended-plugin').click(function () {
        var id = jQuery(this).attr('id'),
            action = jQuery(this).attr('data-action');
        jQuery.ajax({
            type: 'GET',
            data: {action: 'mts_customizer_notify_dismiss_recommended_plugins', id: id, todo: action},
            dataType: 'html',
            url: tiCustomizerNotifyObject.ajaxurl,
            beforeSend: function (data, settings) {
                jQuery('#' + id).parent().append('<div id="temp_load" style="text-align:center"><img src="' + tiCustomizerNotifyObject.base_path + '/images/spinner-2x.gif" /></div>');
            ...
            success: function (data) {
                ...
                if (jQuery('.recomended-actions_container > .epsilon-recommended-plugins').length === 0) {
                    jQuery('.control-section-mts-customizer-notify-recomended-section').remove();
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                console.log(jqXHR + ' :: ' + textStatus + ' :: ' + errorThrown);
